...
<div class="navbar navbar-inverse navbar-fixed-top">
  ...
  <div class="navbar-inner">
    ...
    <div class="container">
      ...
      <form class="navbar-search pull-left" action="${domain}/admin/search">
  ...
  <ul class="portfolio-grid">
    ...
    <c:forEach items="${productsParam.prod_list}" var="item">
      <li class="grid-item" data-jkit="[show:delay=500;speed=500;animation=fade]">
        ...
        <c:if test="${item.is_top > 0}"><a class="btn btn-mini" href="${domain}/admin/untop/${item.id}">Un Top</a></c:if>
        <c:if test="${item.is_top == 0}"><a class="btn btn-mini" href="${domain}/admin/settop/${item.id}">Top</a></c:if>

        <a class="btn btn-mini" href="${domain}/admin/update/${item.id}">Edit</a>
        <a class="btn btn-mini btn-danger" href="${domain}/admin/delete/${item.id}">Del</a>
      </li>
    </c:forEach>
    ...
    <div class="text-intro">
      <div class="pagination">
        ...
        %><li><a href="${domain}/admin/search/<%=i %>?searchinfo=<%=pageParam.getSubTitle() %>"><%=i %>  </a></li><%
            }
          }
        %>
        </ul>
